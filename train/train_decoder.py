...
def train_decoder(
    ...
    print(f"Training {save_dir}")
    running_loss = 0
    t = tqdm(enumerate(dataloader), total=len(dataloader), disable=(rank != 0))
    for i, batch in t:
        targets, labels2d, labels1d = batch
        inputs = labels2d if latent_dim == 2 else labels1d
        ...
        if (rank == 0) and ((i + 1) % 16 == 0):
            ...
            t.set_description_str(f"Training Decoder... Loss={avg_loss:.4f}")
    ...
    if rank == 0:
        ...
        model_copy = model_class(out_dim=latent_dim, img_size=img_size, device=device).to(device)
        model_copy.load_state_dict(state_dict)

        # Trace and save the model
        model_copy.eval()
        scripted_model = torch.jit.trace(model_copy, torch.rand(1, latent_dim).to(device))
