...
def train_process(rank, c: TrainRunConfig):
  ...
  try:
    ...
    if c.run is not None:
      c.run.summary["time_taken"] = time.time() - start_time
      c.run.finish()
...
def _train(c: TrainRunConfig):
    ...
    dataloader = DataLoader(dataset, batch_size=c.batch_size, sampler=sampler, pin_memory=True, drop_last=True, num_workers=6) # increase num_workers if GPU is underutilized
    
    total_steps = len(dataloader) * n_epochs
    
    
    # Create validation set as set of 16 random batches. Shape (16, B, C, H, W)
    if c.rank == 0:
      val_sampler = torch.utils.data.RandomSampler(dataset, replacement=True, num_samples=c.batch_size*16)
      val_dataloader = DataLoader(dataset, batch_size=c.batch_size, sampler=val_sampler, pin_memory=True, drop_last=True, num_workers=4)
    ...
    t = tqdm(total=total_steps, disable=(c.rank != 0))
    ...
    for epoch in range(n_epochs):
      ...
      for i, batch in enumerate(dataloader):
          ...
          if (c.rank == 0) and ((i + 1) % 16 == 0):
              ...
              t.set_postfix(epoch=f"Epoch {epoch+1}/{n_epochs}",batch="{i+1}/{len(dataloader)}", gpus=f"{c.world_size}")
              if c.run is not None:
                  c.run.log({"train_loss": avg_loss, "steps": i + 1})
              running_loss = 0
...
def eval_model(
  ...
  val_loss_train /= n_batches
  
  print(val_loss_eval, val_loss_train)
...
def eval_and_save_model(
  ...
  # Evaluate the model test loss
  val_loss_eval, val_loss_train = eval_model(c, model_copy, device, criterion, val_data)
